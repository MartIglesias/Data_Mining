...
AgregarVariables  <- function( dataset )
{
  ...
  dataset[ , cantidadTotalPrestamos    	:= rowSums( cbind( cprestamos_personales, cprestamos_prendarios, cprestamos_hipotecarios), na.rm=TRUE ) ]
  dataset[ , antiguedadRelativaMayores 	:= round(cliente_antiguedad / (cliente_edad - 18), 2) ]
  dataset[ , antiguedadRelativa 		:= round(cliente_antiguedad / cliente_edad, 2 ) ]
  dataset[ , transaccionesTotalesTarjeta 	:= rowSums( cbind( ctarjeta_debito_transacciones, ctarjeta_visa_transacciones, ctarjeta_master_transacciones), na.rm=TRUE ) ]
  dataset[ , transaccionesTotales 	:= rowSums( cbind( ctarjeta_debito_transacciones, ctarjeta_visa_transacciones, ctarjeta_master_transacciones, tcambio_monedas), na.rm=TRUE ) ]
  dataset[ , interaccionBanco 		:= rowSums( cbind( ccallcenter_transacciones, chomebanking_transacciones, cautoservicio_transacciones, ccajeros_propio_transacciones, ccajeros_ajenos_transacciones), na.rm=TRUE ) ]
  dataset[ , cajerosPropiosAjenos         := round(ccajeros_propio_transacciones / (ccajeros_ajenos_transacciones + 1), 2) ]
  dataset[ , sueldoEstimado 		:= rowSums( cbind( mplan_sueldo_manual, mplan_sueldo), na.rm=TRUE ) ]
  dataset[ , debitosAutomaticosSueldo     := round(mcuenta_debitos_automaticos / (sueldoEstimado + 1), 2) ]
  dataset[ , inversiones                  := rowSums( cbind(mplazo_fijo_dolares, mplazo_fijo_pesos, mfondos_comunes_inversion_pesos, mfondos_comunes_inversion_dolares, mcambio_monedas_compra), na.rm=TRUE ) ]
  dataset[ , inversionesIngresos          := round(inversiones  / (sueldoEstimado + 1 ), 2)]
  dataset[ , segurosTotales               := rowSums( cbind( tseguro_vida_mercado_abierto, tseguro_auto, tseguro_vivienda, tseguro_accidentes_personales), na.rm=TRUE) ]
  dataset[ , seguroBool                   := segurosTotales > 0]
  dataset[ , mcaja_ahorro_relativa                := round( mcaja_ahorro_Paquete / mcuentas_saldo)]
  dataset[ , transacccionesByPlataCajaAhorro      := round( transaccionesTotales / mcaja_ahorro_Paquete, 2)]
  dataset[ , totalMovimientos                     := rowSums( cbind(tmovimientos_ultimos90dias, transaccionesTotalesTarjeta), na.rm=TRUE ) ]
  dataset[ , visaEstadoByPlataCajaAhorro          := round( Visa_cuenta_estado / mcaja_ahorro_Paquete, 2 ) ]
  dataset[ , visaEstadoByTransaccionesTotales     := round( Visa_cuenta_estado / transaccionesTotalesTarjeta, 2 ) ]
  dataset[ , avgConsumoVisaByTransaccionesTotales := round( mtarjeta_visa_consumo / transaccionesTotalesTarjeta, 2) ]
  dataset[ , mAhorroByDescubierto                 := round( mcaja_ahorro_Paquete / mdescubierto_preacordado, 2) ]
  dataset[ , cuentaCorrienteTotal                 := rowSums( cbind( mcuenta_corriente_Nopaquete, mcuenta_corriente_Paquete), na.rm=TRUE)]
  dataset[ , cuentaCorrienteTotalByDescubierto    := round( cuentaCorrienteTotal / mdescubierto_preacordado, 2)]
  dataset[ , totalMovimientosByCCorriente         := round( totalMovimientos / cuentaCorrienteTotal, 2)]
